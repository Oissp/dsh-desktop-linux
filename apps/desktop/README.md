...
A packaged application checks its target-specific release stream ten seconds after the main window opens; the localized **Check for Updates…** menu item triggers the same check manually. An available release opens one shell-owned confirmation modal. Accepting it waits for an in-flight check, downloads and verifies the Desktop release, stops the dsh child, and hands installation plus restart to electron-updater. The next launch displays the local loading page while reconciling the version-bound runtime.

Shell prompts are modal children of the product window that load their documents from `dsh-app://shell/`, which the protocol handler reads out of the packaged `renderer` directory. Where the platform composites transparent windows, a frameless overlay covers the parent's content behind a dimming scrim. Linux gets an opaque card centered on the parent instead, because an uncomposited transparent window paints its alpha as an opaque backing. The card has no content height of its own, so its document measures itself once laid out and the main process fits the window to that height, bounded below by the controls and above by the parent. The mandatory-update modal offers native move, resize, and maximize, so it uses a framed window everywhere except macOS.

Shell documents follow the application appearance. The preference in `settings.yaml` becomes `nativeTheme.themeSource`, so `prefers-color-scheme` in every shell document follows 设置 → 通用设置 → 外观 rather than the OS, and each document repaints from its own copy of the product's alias-token values. A stored preference naming a custom theme overrides nothing, because only the built-in `light` and `dark` pair has a shell equivalent. Opaque prompt windows are prepainted in the resolved surface color so the document's first paint cannot flash.

Prompt copy is read from the locale dictionary when each prompt opens, not captured at startup: the locale controller resolves the engine's Language preference from `settings.yaml` after the update machinery is constructed, so a captured dictionary would pin every prompt to the system locale.
