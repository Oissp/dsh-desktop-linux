...
Shell prompts are modal children of the product window. Where the platform composites transparent windows, a frameless overlay covers the parent's content behind a dimming scrim. Linux gets an opaque card centered on the parent instead, because an uncomposited transparent window paints its alpha as an opaque backing and would cover the product window with a dark slab. The mandatory-update modal offers native move, resize, and maximize, so it uses a framed window everywhere except macOS.
