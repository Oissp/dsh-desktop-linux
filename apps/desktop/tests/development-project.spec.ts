...
describe('desktop development project', () => {
  it('includes declared workspace packages missing from the hoist directory in the runtime inventory', () => {
    ...
    const project = prepareDevelopmentProject({ projectDir: join(root, 'runtime'), cliDir: cli, hostDir: host, dependencyDir: hoisted, release: release(), target: 'linux-x64' })
    ...
    expect(descriptor).toMatchObject({ platform: 'linux', arch: 'x64' })
