...
const harness = await vi.hoisted(async () => {
  ...
  const nativeTheme = { themeSource: 'system', shouldUseDarkColors: false, on: vi.fn(), removeListener: vi.fn() }
...
describe('desktop main startup', () => {
  ...
  it('hides the application menu on the packaged Linux build where the tray mirrors entries', async () => {
    vi.spyOn(process, 'platform', 'get').mockReturnValue('linux')
    await import('../src/main.ts')
    await harness.preparing.promise
    expect(harness.menu.setApplicationMenu).toHaveBeenCalledWith(null)
    // buildFromTemplate 只允许出现托盘菜单；默认应用菜单（editMenu 等）不得构建。
    const templates = harness.menu.buildFromTemplate.mock.calls.map(call => call[0])
    expect(templates.some(items => items.some(item => 'role' in item && item.role === 'editMenu'))).toBe(false)
  ...
  it.each(['darwin', 'linux'] as const)('adds the product File menu and standard window commands only on macOS (%s)', async (platform) => {
    ...
    if (template === undefined) {
      // 打包的 Linux 版本隐藏顶部菜单栏（托盘镜像入口），不构建应用菜单。
