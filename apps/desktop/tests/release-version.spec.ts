...
describe('desktop shell version extends engine version', () => {
  ...
  it('accepts the engine version extended with a build counter', () => {
    ...
    expect(shellVersionExtendsEngine('0.1.7-alpha.2.3', '0.1.7-alpha.2')).toBe(true)
  })
  ...
  it('rejects unrelated, renamed, invalid, and build-metadata-only versions', () => {
    ...
    // A counter cannot extend a stable engine version: four numeric fields are not semver.
    expect(shellVersionExtendsEngine('1.0.0.3', '1.0.0')).toBe(false)
  })
})
