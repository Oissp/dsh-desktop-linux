...
// The framed modal surface is pinned so the assertions do not depend on the host platform.
vi.mock('../src/update-overlay.ts', () => ({ mandatoryUpdateSurface: () => 'window' }))
...
function setup(platform: NodeJS.Platform = 'darwin') {
  ...
  ui = new DesktopMandatoryUpdateWindow({ overlays: { createMandatory: parent }, preload: 'owned', locale: () => resolveDesktopLocale('zh'),
