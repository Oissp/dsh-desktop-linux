...
// 模块级引用防止 Tray 被 GC 后托盘消失；仅打包的 Linux 构建创建。命名与上游 Windows 托盘
// （main() 内的 DesktopTray）区分，两者互斥出现。
let linuxTray: Tray | undefined
...
function linuxWindowIconPath(appearance: DesktopAppearance): string {
  return join(process.resourcesPath, appearance === 'dark' ? 'icon-white.png' : 'icon-dark.png')
}
function linuxTrayIconPath(appearance: DesktopAppearance): string {
...
function createWindow(preload: string, show = false, primary = false): BrowserWindow {
  const window = new BrowserWindow({
    ...
    ...(app.isPackaged && process.platform === 'linux' ? { icon: linuxWindowIconPath(iconAppearance) } : {}),
...
async function main(): Promise<void> {
  ...
  // 重建 Linux 托盘菜单；创建托盘时赋值，语言或外观变更时调用。
  ...
  const refreshApplicationMenu = (): void => {
    // 打包的 Linux 版本隐藏顶部菜单栏；托盘菜单镜像全部入口（显示主窗口、检查更新、退出）。
  ...
  const appearanceController = new DesktopAppearanceController(desktopSettingsPath(), (appearance) => {
    ...
    linuxTray?.setImage(linuxTrayIconPath(appearance))
    for (const window of BrowserWindow.getAllWindows()) {
      if (!window.isDestroyed()) window.setIcon(linuxWindowIconPath(appearance))
    }
  })
  ...
  // 上游只为 Windows 建托盘（DesktopTray）。Linux 的关窗同样只隐藏窗口，这里补一个托盘
  // 作为回到主窗口的入口，并镜像应用菜单（打包的 Linux 隐藏顶部菜单栏）。
  if (app.isPackaged && process.platform === 'linux') {
    ...
    linuxTray = new Tray(linuxTrayIconPath(iconAppearance))
    linuxTray.setToolTip(app.name)
    // 托盘菜单镜像应用入口：检查更新、退出，另加“显示主窗口”作为关窗隐藏后的恢复入口。
    rebuildTrayMenu()
    linuxTray.on('click', focusPrimaryWindow)
  }
